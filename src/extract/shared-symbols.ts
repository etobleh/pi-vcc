...
const matches = (tools: Set<string>, name?: string): boolean =>
  Boolean(name && tools.has(name.toLowerCase()));

// Declaration regexes across 9 languages
...
export const extractFileAndSymbolData = (
  ...
  const read = new Set<string>(fileOps?.readFiles ?? []);
  const modified = new Set<string>(fileOps?.modifiedFiles ?? []);
  const created = new Set<string>(fileOps?.createdFiles ?? []);
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    if (b.kind === "tool_call") {
      ...
      if (p) {
        const isRead = matches(FILE_READ_TOOLS, b.name);
        const isWrite = matches(FILE_WRITE_TOOLS, b.name);
        const isCreate = matches(FILE_CREATE_TOOLS, b.name);
